...
from scripts.data.prepare_division_traces import DivisionTraceConfig, format_division_trace, random_examples  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fast teacher-forced division trace evaluation.")
    ...
    parser.add_argument("--task-prefix", default=None, help="Optional leading task line, e.g. 'Task: division_prose'.")
...
def main() -> None:
    ...
    pairs = random_examples(
        DivisionTraceConfig(
            ...
            seed=args.seed,
            min_dividend_digits=2,
            max_dividend_digits=3,
            min_divisor_digits=1,
            max_divisor_digits=2,
            include_zero_remainder=True,
        )
    )
    traces = [
        tag_trace(
            format_division_trace(dividend, divisor),
            args.task_prefix,
            shared_output_format=args.shared_output_format,
            question_only=args.question_only,
            sentinel=args.sentinel,
        )
        for dividend, divisor in pairs[: args.examples]
    ]
